...
public class AutoAimingDriveOp extends LinearOpMode {
      ...
      while (opModeIsActive()) {
        ...
        //     }
            ...
            for (int i = 0; i < blocks.length; i++) {
                ...
                pos = 1;
                if(blocks[i].y<119){pos=0;}
                if(blocks[i].y>139){pos=2;}
                angleCorrect = 1;
                if(blocks[i].height-blocks[i].top>0){angleCorrect=0;}
                if(blocks[i].height-blocks[i].top<0){angleCorrect=2;}
        ...
        if(gamepad1.rightBumperWasPressed()){useCamera = !useCamera;}
